...
const useDynamicDispatch = ({ useDispatch: useAliasDispatch = useDispatch } = {}) => {
  ...
  return useCallback(
    (actionValue, actionPayload) => {
      if (actionValue && actionPayload) {
        const updatedActionValue = (Array.isArray(actionValue) && joinSelector(actionValue)) || actionValue;

        if (helpers.isPromise(actionPayload)) {
          return dispatch({
            type: updatedActionValue,
            payload: actionPayload,
            meta: {
              __originalType: updatedActionValue,
              __dynamic: true
            }
          });
        }

        const updatedActionPayload = (_isPlainObject(actionPayload) && actionPayload) || { value: actionPayload };
