 ...
 * The _metricId passed via configuration should be removed in favor of the transformer pattern be used by "rhsmTally" below.
 */
/**
...
const rhsmCapacity = (response, { _metricId } = {}) => {
  ...
  updatedResponse.data = data.map(
    (
      ...
      x: index,
      y: hasInfiniteQuantity === true ? null : value,
      date,
      hasInfiniteQuantity,
  ...
  updatedResponse.meta = {
    count: meta[RHSM_API_RESPONSE_META_TYPES.COUNT],
    productId: meta[RHSM_API_RESPONSE_META_TYPES.PRODUCT]
  };
