...
// import inventoryGuestsQuery from "../../redux/reducers/viewReducer";
// import initialGraphFilters from "./productViewColumn";
...
/*
const useProductQueryFactory = (queryType, querySchema) => {
  const { [queryType]: initialQuery, productId, viewId } = useContext(ProductContext) || {};
  const queryProduct = storeHooks.reactRedux.useSelector(({ view }) => view?.[queryType]?.[productId]);
  const queryView = storeHooks.reactRedux.useSelector(({ view }) => view?.[queryType]?.[viewId]);
  ...
  if (querySchema) {
    updatedQuery = reduxHelpers.setApiQuery(updatedQuery, querySchema);
  }
