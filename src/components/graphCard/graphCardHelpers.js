...
const xAxisTickFormat = ({ callback, chartWidth, date, granularity, nextDate, previousDate, tick } = {}) => {
  ...
  switch (granularity) {
    case GRANULARITY_TYPES.QUARTERLY:
      if (chartWidth < 935) {
        // isYearStartEnd = getIsNewYear(nextDate);
        /*
         * isYearStartEnd =
         *  nextDate &&
         *  previousDate &&
         *  Number.parseInt(momentDate.year(), 10) !== Number.parseInt(moment.utc(nextDate).year(), 10);
         */
        /*
         * formattedDate = momentDate.format(dateHelpers.timestampQuarterFormats.yearShort);
         * formattedDate = momentDate.format("MMM 'YY");
         */
      } else {
        /*
         * formattedDate = isYearStart || chartWidth < 935
