...
const useRedirect = ({
  ...
  return useCallback(
    route => {
      ...
      // window.location.replace((isUrl && route) || `${routerHelpers.pathJoin(baseName, route)}${search}${hash}`);
      ...
      window.location.href = (isUrl && route) || `${routerHelpers.pathJoin(baseName, route)}`;
      return undefined;
    },
    [hash, search]
  );
